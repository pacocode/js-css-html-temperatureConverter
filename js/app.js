...
const update = (e) => {
    ...
    if (elementId === 'secondary-input') {
        primaryInput.value = calculate(secondarySelect.value, primarySelect.value, primaryInput.value);
    } else {
        secondaryInput.value = calculate(primarySelect.value, secondarySelect.value, primaryInput.value)
    }
}

const calculate = (firstTempUnit, secondTempUnit, temp) => {
    ...
    switch (combination) {
        // convert fanrenheit to get celcius
...
/*
    //First way to add event listener  to he controls
...
// Another way to add listener to the controls
[primaryInput, secondaryInput, primarySelect, secondarySelect].forEach(element => element.addEventListener('change', update))